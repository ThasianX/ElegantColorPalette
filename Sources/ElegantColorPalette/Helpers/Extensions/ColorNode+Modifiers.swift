...
public extension ColorNode {
    ...
    /// Adds or removes the name label to this node
    ///
    /// - Parameter active: Whether or not to show the label of this node.
    /// - Returns: A node that is/isn't showing its label.
    func label(_ active: Bool) -> ColorNode {
        modifier(LabelModifier(show: active))
    }
    ...
    /// Adds or removes the border to this node.
    ///
